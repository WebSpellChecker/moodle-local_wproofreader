 ...
 * OptionsManager defaults). It sets this to "true" once an instance is fully
 * attached - but only on the container it directly manages. For a plain
 * textarea (autoSearch-attached, container === the textarea) that is the
 * field itself, so it is a reliable outer-page signal there. For TinyMCE the
 * container we pass is the outer iframe, but the bundle tags something
 * inside the iframe's own document instead, so this never appears on the
 * iframe element itself; Atto and TinyMCE rely on ATTACHED_ATTR below
 * instead.
 */
